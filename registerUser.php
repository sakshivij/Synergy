...
try {
    ...
    if($event == 'Athletics'){
		...
		$stmt = $conn->prepare("INSERT INTO ignitesynergy_details.User(email,name,event,contact,meter100,p100,meter200,p200,meter400,p400,meter800,p800,meter1500,p1500,meter4100,p4100,meter4200,p4200,longJump,plong,shortPut,pshort,college) 
    	VALUES (:email,:name,:event,:contact,:meter100,:p100,:meter200,:p200,:meter400,:p400,:meter800,:p800,:meter1500,:p1500,:meter4100,:p4100,:meter4200,:p4200,:longJump,:long,:shortPut,:short,:college)");
		...
		$stmt = $conn->prepare("INSERT INTO ignitesynergy_details.User(email,name,event,contact,pevent,college) 
    	VALUES (:email,:name,:event,:contact,:pevent,:college)");
		$stmt->bindParam(':email', $email);
    	$stmt->bindParam(':name', $name);
    	$stmt->bindParam(':event', $event);
    	$stmt->bindParam(':contact', $contact);
...
//echo "Executing";
    ...
    //email config
	 ...
	 echo email("synergy.igdtuw@gmail.com",$name,$student_one,"PHPMailer/realestate/index2.php",$cch);
